...
function openModalStruktur(jenis,data=null) {
  ...
  const titles={bpmj:'Anggota BPMJ',pelayan:'Pelayan Khusus',komisi:'Komisi Kerja & Lansia'};
...
function exportExcel(mode){
  ...
  const data=sumber.map((j,i)=>({'No':i+1,'Kolom':j.kolom||'','No Urut':j.no_urut||'','Nama Lengkap':j.nama_lengkap||'','NIK':j.nik||'','L/P':j.lp||'','Tempat Lahir':j.tempat_lahir||'','Tanggal Lahir':formatTanggal(j.tanggal_lahir),'Umur':hitungUmur(j.tanggal_lahir),'Pekerjaan':j.pekerjaan||'','Status Baptis':j.baptis==='sudah-baptis'?'Sudah Baptis':'Belum Baptis','Status Sidi':j.sidi==='sudah-sidi'?'Sudah Sidi':'Belum Sidi','Nama Keluarga':j.nama_keluarga||'','No KK':j.no_kk||'','Relasi':j.relasi||'','Tanggal Nikah':j.tanggal_nikah?formatTanggal(j.tanggal_nikah):'','Kategori BIPRA':j.bipra||'','Lansia':j.lansia==='lansia'?'Lansia':'','Jemaat Asal':j.jemaat_asal||'','Alamat Rumah':j.alamat_rumah||'','Alamat Kolom':j.alamat_kolom||'','Status Jemaat':j.status_jemaat==='baru'?'Jemaat Baru':'Jemaat Lama'}));
  if (!data.length){alert('Tidak ada data');return;}
