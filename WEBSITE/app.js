...
function closeModalPengumuman(){document.getElementById('modalPengumuman').classList.remove('open');}

async function editPengumuman(id) {
  const {data}=await sb.from('pengumuman').select('*').eq('id',id).single();
  if (data) openModalPengumuman(data);
}

async function deletePengumuman(id) {
  if (!confirm('Hapus pengumuman ini?')) return;
  await sbAdmin.from('pengumuman').delete().eq('id',id);
  showToast('Pengumuman dihapus','success'); loadPengumumanAdmin();
}
...
async function savePengumuman() {
  ...
  const payload={judul,isi,tanggal_mulai:document.getElementById('pMulai').value||null,tanggal_selesai:document.getElementById('pSelesai').value||null,aktif:document.getElementById('pAktif').value==='true',dibuat_oleh:currentUser?.username||'-'};
  const result=id?await sb.from('pengumuman').update(payload).eq('id',id):await sb.from('pengumuman').insert(payload);
  if (result.error){showToast('Gagal simpan: '+result.error.message,'error');return;}
  showToast('Pengumuman berhasil disimpan ✅','success');
  await kirimNotifikasiEmail(id?'edit':'tambah','Pengumuman',`${id?'Edit':'Tambah'} Pengumuman: "${judul}"`);
